...
int main(int, char **)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		float radius = 10.0f;
		float zCameraPos = cos((float)glfwGetTime()) * radius;
		float xCameraPos = sin((float)glfwGetTime()) * radius;
		glm::mat4 view;
		view = glm::lookAt(glm::vec3(xCameraPos, 0.0f, zCameraPos),
							glm::vec3(0.0f, 0.0f, 0.0f),
							glm::vec3(0.0f, 1.0f, 0.0f));

		// create transformations
		// glm::mat4 view = glm::mat4(1.0f); // make sure to initialize matrices to identity matrix first
...
void handleInput(GLFWwindow *window)
{
	...
	// Set wireframe view mode on 'W' key press
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS)
	{
		glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
	}

	// Set fill view mode on 'F' key press
